...
function SiaNode(core, nexus, uuid, ident) {
    ...
    self.updatePrice = function(_SC_TB_MO) {
        ...
        // var PRICE = SC_GB_MO * 1e24 / 1e9 / 4320;
        ...
        /*
        self.sia.host.post({
            minstorageprice : PRICE.toString()
        }, function(err) {
            if(err && err.error)
                err = err.error;
            console.log("price update".bold+' --> '+self.uuid+': '+(err ? err.toString().magenta.bold : " Ok".green.bold))
        })
        */        
    }
    ...
    self.on('host-state', function(state) {
        self.state.host = state;
        //        console.log("HOST STATE", args.data);
    })

    self.on('node-state', function(state) {
        self.state.node = state;
        //        console.log("NODE STATE", args);
    })
...
function Nexus(core) {
    ...
    //self.PRICE_FACTOR = 0.9;
    //self.USE_MARKET_PRICE = true;
    self.pricing = { }
    //self.priceAggregatorFreq = 1 * 60 * 1000;
    ...
    self.priceAggregator = new PriceAggregator(core, self.markets, self.sia, { 
        //freq : self.priceAggregatorFreq,
        TARGET_USD_TB_MO : initSettings.cluster.TARGET_USD_TB_MO,
        MARKET_USD_PEG : initSettings.cluster.MARKET_USD_PEG,
    ...
    self.priceAggregator.registerSink(function(pricing, callback) {
        ...
        // self.hostStorage = self.priceAggregator.hostStorage;
        self.pricing = pricing;

        //var SC_TB_MO = Math.floor(pricing.SC_TB_MO);

        self.notifyUI("price-update", pricing);
        if (pricing.AVG_SC_TB_MO && pricing.AVG_SC_TB_MO != "N/A") {
            self.stats.push("cluster", {
    ...
    self.updateExplorer = function(callback) {
        ...
        // TODO - Migrate this to self.sia.explorer() once API becomes available (?) can't seem to access GET /explorer 
        ...
        request({
            ...
            body.totalcoins = body.totalcoins / 1e24;
            body.totalcontractcost = body.totalcontractcost / 1e24;
    ...
    core.init(function(callback) {
        ...
        self.rpc = new irisRPC.Multiplexer({
            uuid : core.uuid,
            //port: core.config.rpc.node.port,
            //auth: core.config.rpc.node.auth,
        ...
        self.rpc.on('node-state', function(msg, uuid) {
            if (!self.nodes[uuid])
                return;
            msg.uuid = uuid;
            //console.log("NODE-STATE".green.bold, uuid, msg.data);
            ...
            // ---- @asy  we are saving fields under node only if those dont goes into global(cluster.fields for now, will move to global.fields) fields
        ...
        function mergeNodeFields(nodeId, fields, settings){
            ...
            if (settings[nodeId] && settings[nodeId].fields){
                s = settings[nodeId];
                var oldSettings = settings[nodeId].fields;
                var setupFields = setupSettings && setupSettings.cluster && setupSettings.cluster.fields;
                var _fields = {};
                _.each(setupFields, function(v, key){
                    if (oldSettings[key]){
                        _fields[key] = oldSettings[key];
                    }else{
                        _fields[key] = v;
                    }
                })
        ...
        self.rpc.dispatch(uuid, data, function(err, result){
            ...
            if (err && _.isString(err)){
                if (err.indexOf("Status Code Returned")){
                    if (type == "direct"){
                        err = "Make sure Passphrase is correct.";
                    }else{
                        err = "Make sure Walletkey is correctly configured under advanced settings.";
                    }
                }
                err = {error: err};
            }
    ...
    core.on("ws::setting", function(args, callback){
        ...
        if (fields[name]){//if it is sia-field/setting field  ----------  @asy
    ...
    core.on("ws::get-cluster-stats", function(args, callback){
        ...
        //var endTS   = Date.now();
        //var startTS = endTS - 2 * daySeconds;
        ...
        self.stats.get("cluster", names, args.startTS, args.endTS, points, function(err, items){
            if (err)
                return callback(err);
            //console.log("memory:items".greenBG, items)
            result.SC_TB_MO.data = items;
            
            return callback(null, result);
            /*
            names = {
                SC_BTC: function(item){ return {name: "SC / uBTC", value: item.value * 1e6}},
                ts: "x"
            };

            self.stats.get("cluster", names, startTS, endTS, points, function(err, items){
                if (err)
                    return callback(err);
                //console.log("load:items".greenBG, items)
                result.SC_BTC.data = items;

                names = {
                    SC_USD: function(item){ return {name: "SC / Cent", value: item.value * 100}},
                    ts: "x"
                };
    ...
    core.on("ws::get-storage-stats", function(args, callback){
        ...
        //var settings = getSettings();
        _.each(self.nodes, function(node) {
            var state = node.state.node.host;
            //if (!state && settings[node.uuid] && settings[node.uuid].data && settings[node.uuid].data.host)
                //state = settings[node.uuid].data.host


            //state && state.externalsettings && console.log("state.externalsettings".greenBG, state.externalsettings)
    ...
    core.on("ws::get-variable-graph-data", function(args, callback){
        ...
        self.stats.get(uuid, names, startTS, endTS, points, function(err, items){
            if (err)
                return callback(err);
            result.data = items;
            callback(null, result);

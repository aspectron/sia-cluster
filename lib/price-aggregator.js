...
function PriceAggregator(core, markets, sia, config) {
	var self = this;
	...
	self.pricing = {
		...
		MARKET_USD_PEG : (config.MARKET_USD_PEG === undefined ? true : config.MARKET_USD_PEG),
...
//		})
	...
	self.updateHostAverages = function(callback) {
        ...
        self.sia.renter.hosts.active(function(err, resp) {
            ...
            if(err || !resp.hosts) {
                ...
                hostList = _.sortBy(hostList, 'totalstorage');
                ...
                self.pricing.PEERS_TOTAL = hostList.length;
